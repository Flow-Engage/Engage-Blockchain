...
pub contract Engage_2: NonFungibleToken {

    // Engage_2 contract events
	...
	// -----------------------------------------------------------------------
    // Engage_2 contract-level fields.
    // -----------------------------------------------------------------------
   ...
   // access(self) var categoriesDatas: {UInt64: Category}
    ...
    // TotalSupply to keep track of all the Engage_2 NFTs 
    // ever minted
    ...
    // -----------------------------------------------------------------------
    // Engage_2 contract-level Composite Type definitions
    // -----------------------------------------------------------------------
    ...
    pub resource Platform {
        ...
        init(name: String) {
            ...
            self.platformID = Engage_2.nextPlatformID
            self.categories = {}
			self.name = name

            // Create a new PlatformData for this Platform and store it in contract storage
 //           Engage_2.platformDatas[self.platformID] = PlatformData(name: name)
        }
        ...
        pub fun addCategory(categoryName: String): UInt64 {
            ...
            Engage_2.categories[newCategoryID] <-! newCategory
        ...
        pub fun addMatchToCategory(categoryName: String, matchName: String): UInt64 {
            ...
            let categoryRef = (&Engage_2.categories[categoryID] as &Category?)!
	...
	pub resource Category {
		...
		init(name: String, platformID: UInt64, platformName: String) {
			...
			self.matches = {}
			self.categoryID = Engage_2.nextCategoryID

            // Increment the ID so that it isn't used again
            Engage_2.nextCategoryID = Engage_2.nextCategoryID + 1
		}
        ...
        pub fun addMatch(matchName: String): UInt64 {
            ...
            Engage_2.matches[newMatchID] <-! newMatch
	...
	pub resource Match {
		...
		init(name: String, categoryName: String, categoryID: UInt64, platformName: String, platformID: UInt64) {
			...
			self.matchID = Engage_2.nextMatchID

            // Increment the ID so that it isn't used again
            Engage_2.nextMatchID = Engage_2.nextMatchID + 1        
		}
        ...
        pub fun mintNFTs(
            ...
            Engage_2.nftMetadatas[Engage_2.nextMetadataID] = NFTMetadata(
                _metadataID: Engage_2.nextMetadataID,
                _name: name,
            ...
            var i: UInt64 = 0
            while i < quantity {
                newCollection.deposit(token: <- create NFT(_metadataID: Engage_2.nextMetadataID))
                i = i + 1
            }
            // Increment the ID so that it isn't used again
            Engage_2.nextMetadataID = Engage_2.nextMetadataID + 1
    ...
    // let platformData = Engage_2.QueryPlatformData(platformID: 12)
    //
    pub struct QueryPlatformData {
        ...
        init(platformID: UInt64) {
            pre {
                Engage_2.platforms[platformID] != nil: "The Platform with the provided ID does not exist"
            }

            let platformRef = (&Engage_2.platforms[platformID] as &Platform?)!
    ...
    pub struct QueryCategoryData {
        ...
        init(categoryID: UInt64) {
            pre {
                Engage_2.categories[categoryID] != nil: "The Category with the provided ID does not exist"
            }
    ...
    // The resource that represents the Engage_2 NFTs
    //
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
        // Global unique Engage_2 ID
        pub let id: UInt64
        // Serial number of this NFT
        pub let serial: UInt64
        // Struct of Engage_2 NFTs metadata
        pub let metadataID: UInt64
        ...
        init(_metadataID: UInt64) {
            pre {
				    Engage_2.nftMetadatas[_metadataID] != nil: "This Metadata doesn't exist"
			    }
            // Increment the global Engage_2 IDs
            // Gotta change this to UUID in the case that we implement burning
            Engage_2.totalSupply = Engage_2.totalSupply + 1
            // Assign serial number to the NFT based on the number of minted NFTs
			let metadataRef: &Engage_2.NFTMetadata = (&Engage_2.nftMetadatas[_metadataID] as &NFTMetadata?)!
            self.id = Engage_2.totalSupply
            self.metadataID = _metadataID
        ...
        pub fun getMetadata(): NFTMetadata {
			return Engage_2.getNFTMetadata(self.metadataID)!
		}
                ...
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([
                        MetadataViews.Royalty(
                            ...
                            description: "The Engage_2 address gets 5% of every secondary sale."
                        )
                    ])
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let bannerImage = MetadataViews.Media(
                        file: MetadataViews.HTTPFile(
                            url: "https://nEngage_2.com/static/img.svg"
                        ),
                        mediaType: "image/svg+xml"
                    )
                    let squareImage = MetadataViews.Media(
                        file: MetadataViews.HTTPFile(
                            url: "https://Engage_2.com/static/img.png"
                        ),
                        mediaType: "image/png"
                    )
                    return MetadataViews.NFTCollectionDisplay(
                        name: "Engage_2",
                        description: "Engage_2 description TBD",
                        externalURL: MetadataViews.ExternalURL("https://Engage_2.com"),
                        squareImage: squareImage,
                        bannerImage: bannerImage,
                        socials: {
                            "twitter": MetadataViews.ExternalURL("https://twitter.com/Engage_2"),
                            "discord": MetadataViews.ExternalURL("https://discord.com/invite/Engage_2"),
                            "instagram": MetadataViews.ExternalURL("https://www.instagram.com/Engage_2")
                        }
                    )
    ...
    //
    pub resource interface Engage_2CollectionPublic {
	...
	pub resource Collection: Engage_2CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
		...
		pub fun claim() {
			if let storage = &Engage_2.nftStorage[self.owner!.address] as &{UInt64: NFT}? {
    ...
    // various aspects of the Engage_2 contract
    //
	pub resource Administrator {
		// createPlatform creates a new Platform resource and stores it
        // in the platforms mapping in the Engage_2 contract
        //
        ...
        pub fun createPlatform(_name: String): UInt64 {
            ...
            // Increment the ID so that it isn't used again
            Engage_2.nextPlatformID = Engage_2.nextPlatformID + 1

            emit PlatformCreated(id: newPlatform.platformID, name: _name)

            // Store it in the platforms mapping field
            Engage_2.platforms[newPlatformID] <-! newPlatform

            return newPlatformID
        }

        // borrowPlatform returns a reference to a Platform in the Engage_2
        ...
        pub fun borrowPlatform(PlatformID: UInt64): &Platform {
            pre {
                Engage_2.platforms[PlatformID] != nil: "Cannot borrow Platform: The Platform doesn't exist"
            }
            
            // Get a reference to the Platform and return it
            return (&Engage_2.platforms[PlatformID] as &Platform?)!
        }

		// createCategory creates a new Category resource and stores it
        // in the category mapping in the Engage_2 contract
        //
		...
		pub fun createCategory(_name: String, _platformID: UInt64):UInt64 {
            pre {
                Engage_2.platforms[_platformID] != nil: "This Platform doesn't exist"
            }
            // Add new Category to the selected Platform
            let platformRef = (&Engage_2.platforms[_platformID] as &Platform?)!
        ...
        pub fun createNFTs(
            ...
            pre {
                Engage_2.matches[_matchID] != nil: "This Match doesn't exist"
            }

            let matchRef = (&Engage_2.matches[_matchID] as &Match?)!
    ...
    // -----------------------------------------------------------------------
    // Engage_2 contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun getPlatformData(_platformID: UInt64): QueryPlatformData? {
        if Engage_2.platforms[_platformID] == nil {
            return nil
        } else {
    ...
    pub fun getCategoryData(_categoryID: UInt64): QueryCategoryData? {
        if Engage_2.platforms[_categoryID] == nil {
            return nil
        } else {
...
//		self.categoriesDatas = {}
		...
		self.CollectionStoragePath = /storage/Engage_2Collection
		self.CollectionPublicPath = /public/Engage_2Collection
		self.CollectionPrivatePath = /private/Engage_2Collection
		self.AdministratorStoragePath = /storage/Engage_2Administrator
		...
		self.account.link<&Collection{Engage_2CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(
